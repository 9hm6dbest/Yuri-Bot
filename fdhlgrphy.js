...
module.exports = fdhl = async (fdhl, mech, _welkom) => {
	try {
        if (!mech.hasNewMessage) return
        mech = mech.messages.all()[0]
		if (!mech.message) return
		if (mech.key && mech.key.remoteJid == 'status@broadcast') return
		global.blocked
		global.prefix
        	mech.message = (Object.keys(mech.message)[0] === 'ephemeralMessage') ? mech.message.ephemeralMessage.message : mech.message
        const content = JSON.stringify(mech.message)
		const from = mech.key.remoteJid
		const { text, extendedText, contact, contactsArray, groupInviteMessage, listMessage, buttonsMessage, location, liveLocation, image, video, sticker, document, audio, product, quotedMsg } = MessageType
		const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
        const type = Object.keys(mech.message)[0]        
        const cmd = (type === 'conversation' && mech.message.conversation) ? mech.message.conversation : (type == 'imageMessage') && mech.message.imageMessage.caption ? mech.message.imageMessage.caption : (type == 'videoMessage') && mech.message.videoMessage.caption ? mech.message.videoMessage.caption : (type == 'extendedTextMessage') && mech.message.extendedTextMessage.text ? mech.message.extendedTextMessage.text : ''.slice(1).trim().split(/ +/).shift().toLowerCase()         	
        body = (type === 'conversation' && mech.message.conversation.startsWith(prefix)) ? mech.message.conversation : (type == 'imageMessage') && mech.message[type].caption.startsWith(prefix) ? mech.message[type].caption : (type == 'videoMessage') && mech.message[type].caption.startsWith(prefix) ? mech.message[type].caption : (type == 'extendedTextMessage') && mech.message[type].text.startsWith(prefix) ? mech.message[type].text : (type == 'listResponseMessage') && mech.message[type].singleSelectReply.selectedRowId ? mech.message[type].singleSelectReply.selectedRowId : (type == 'buttonsResponseMessage') && mech.message[type].selectedButtonId ? mech.message[type].selectedButtonId : ''
		budy = (type === 'conversation') ? mech.message.conversation : (type === 'extendedTextMessage') ? mech.message.extendedTextMessage.text : ''
		const command = body.slice(1).trim().split(/ +/).shift().toLowerCase()		
		const args = body.trim().split(/ +/).slice(1)
		const isCmd = body.startsWith(prefix)
		const q = args.join(' ')
		const Verived = "0@s.whatsapp.net"
		const txt = mech.message.conversation
		const botNumber = fdhl.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6283862323152@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mech.participant : mech.key.remoteJid
		...
		const conts = mech.key.fromMe ? fdhl.user.jid : fdhl.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mech.key.fromMe ? fdhl.user.name : conts.notify || conts.vname || conts.name || '-'        
		...
		const isMybot = isOwner || mech.key.fromMe
		//INI TIME TOD
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			...
			success: 'Its Done bro ',
			error: {
				stick: 'Convert To Sticker...Just be paitiont!',
				Iv: 'The link is an error!'
			},
			only: {
				admin: 'only Admins bro!',
				group: 'Special for Group bro!'
			}
		}
		fakeText = wmaing
            const isUrl = (ini) => {
            ...
            const reply = (Text) => {
                fdhl.sendMessage(from, Text, text, { quoted: mech })
            }
            const sendMess = (hehe, Text) => {
                fdhl.sendMessage(hehe, Text, text)
            }
            ...
            const nigambar = fs.readFileSync ('./Arin/commands.jpg')
            const mentions = (Text, memberr, id) => {
                (id == null || id == undefined || id == false) ? fdhl.sendMessage(from, Text.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }): fdhl.sendMessage(from, Text.trim(), extendedText, { quoted: mech, contextInfo: { "mentionedJid": memberr } })
            }
            async function faketoko(Text, url_image, title, code, price) {
                ...
                await fdhl.sendMessage(from, Text, text, ini_cstoko)
            }        
//━━━━━━━━━━━━━━━[ BUTTON BENTAR BANG ]━━━━━━━━━━━━━━━━━//

        const sendButton = async (from, context, fortext, but, mech) => {
            buttonMessages = {
                contentText: context,
                footerText: fortext,
                buttons: but,
        ...
        const sendButImage = async (from, context, fortext, img, but, mech) => {
        ...
        //sendButLoc(id/from, "string", "string", image, but, mech)
...
//━━━━━━━━━━━━━━━[ FAKE RPLY ]━━━━━━━━━━━━━━━━━//
        const fakestatus = (Text) => {
            fdhl.sendMessage(from, Text, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        fdhl.sendMessage(to, media, MessageType.sticker,{quoted:mech})
...
reply(` *「 GROUP LINK DETECTOR 」*\nwe have detected another chat group link, and sorry we have to kick you out of the group :)`)
setTimeout(() => {
fdhl.groupRemove(from, [kic]).catch((e) => { reply(`Cannot kick group admins`) })
}, 0)
}
		if (budy.length > 3500) {
if (!isGroup) return
...
if (isGroupAdmins) return
reply('Mark as read\n'.repeat(300))
reply(`「 *VIRTEX DETECTOR* 」\n\nwe have detected a virtex, and sorry wa have to kick you out of the group :)`)
console.log(color('[KICK]', 'red'), color('Received a virus text!', 'yellow'))
fdhl.groupRemove(from, [sender])
}     
if (budy.includes(`السلام عليكم`)) {
reply(`وعليكم السلام يا اخي ${pushname}`)
}
if (budy.includes(`Hi Yuri`,`اهلا يوري`)) {

reply(`اهلا *${pushname}😁*\nانا يوري كوروياناغي هل استطيع مساعدتك  في اي شيء؟\nاكتب .menu حتي تري قائمة الاوامر`)
}
...
//┅┅┅┅〔 KALO ADA BUG LAPORKAN!! 〕┅┅┅┅/
			case 'blackpink2': 
                    if (args.length < 1) return reply(`Where's the text bro? *${pushname}?*😒\nExample ${prefix}${command} Dark Bot`)
					mak = body.slice(11)
					reply(mess.wait)
					...
					fdhl.sendMessage(from, buffer1, image, {quoted: mech, thumbnail: fs.readFileSync('./pebrikun.jpg')})
					break 
			case 'neon': 
                    if (args.length < 1) return reply(`Where's the text bro? *${pushname}?*😒\nExample ${prefix}${command} Dark Bot`)
					makell = args.join(" ")
			...
			case 'naruto': 
					...
					break 
	       case 'Write':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
           getBuffer(`http://hadi-api.herokuapp.com/api/canvas/Write?text=${ini_txt}`).then((gambar) => {
           fdhl.sendMessage(from, gambar, image, { quoted: mech })
              })
           break
	       case 'Write2':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/canvas/Write2?text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'Writeleft':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`https://hardianto-chan.herokuapp.com/api/Writeleft?text=${ini_txt}&apikey=${hardiapi}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'narutobanner':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/manga-naruto?Text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break	       
	       case 'fireText':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/flaming-fire?Text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break	       
	       case 'goldText':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/metalic-gold?Text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'skyText':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/shadow-sky?Text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'cupText':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/Text-cup?Text=${ini_txt}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'loveText':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
           ini_txt = args.join(" ")
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/romantic-messages?Text=${body.slice(10)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'burnpaper':
           if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/burn-paper?Text=${body.slice(11)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'loveText2':
	      if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/love-messages?Text=${body.slice(11)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	       case 'loveText3':
	       if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/romantic-double?Text=${body.slice(11)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break	       
	       case 'coffecup':
	       if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/coffee-cup?Text=${body.slice(10)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break	       	       
	       case 'woodhearth':
	       if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/wood-hearth?Text=${body.slice(12)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break	       	       	       
	       case 'grafititext':
	      if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} Darkbot`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/grafiti-text-cover?Text=${body.slice(13)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mech})
	       break
	            case 'blackpink':
                case 'neon':
...
//»»»»»»»»»»»»»»»»»»»»»»|| INI NSRW BANG ||«««««««««««««««««««««««//
           ...
           case 'nsfw':
	        ...
	        if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('Already active Bro')
						nsfww.push(from)
						fs.writeFileSync('./database/nsfww.json', JSON.stringify(nsfww))
						reply('Successfully activate the nsfw feature')
						fdhl.sendMessage(from, `Free to Find Hentai 🗿`, text)
					} else if (Number(args[0]) === 0) {
						if (!isNsfw) return reply('Its dead bro')
...
// MAU NYOLONG? BAGI WM GW TOD!! GAK BAGI SIAP DI BULY
     ...
     case 'cikomodex' :
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner) return reply('FEATURES ONLY AVAILABLE FOR OWNER')
         but = [
         { buttonId: '#Darkbotmodexon', buttonText: { displayText: 'ON' }, type: 1 },
         { buttonId: '#Darkbotmodexoff', buttonText: { displayText: 'OFF' }, type: 1 }
         ]
         sendButton(from, "*PLEASE CHOOSE FOR DarkBot MODE X*", fakeText, but, {quoted: mech})
         break
     case 'cikomodexon' :
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner) return reply('*FEATURES ONLY AVAILABLE FOR OWNER*')
         if (isCikmod) return reply('*Darkbot MODE X Active!!*')
         cikomode.push(from)
         fs.writeFileSync('./database/cikomode.json', JSON.stringify(cikomode))
         reply(`\`\`\`SUCCESSFUL ACTIVATE DARKBOT MODE X IN GROUP\`\`\` *${groupMetadata.subject}*`)
         break
     case 'cikomodexoff' :
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner) return reply('*FEATURES ONLY AVAILABLE FOR OWNER*')
         if (!isCikmod) return reply('*DARKBOT MODE X HAS BEEN ACTIVE BEFORE!*')
         cikomode.splice(from, 1)
         fs.writeFileSync('./database/cikomode.json', JSON.stringify(cikomode))
         reply(`\`\`\`SUCCESSFUL DISABLED DARKBOT MODE X IN GROUP\`\`\` *${groupMetadata.subject}*`)
         break
                        ...
                        .then(async(result) => {
                            await fetchJson(`https://api.lolhuman.xyz/api/ytaudio2?apikey=${cikoapi}&url=https://www.youtube.com/watch?v=${result.result[0].videoId}`)
                                .then(async(result) => {
                                    ...
                                    await fdhl.sendMessage(from, ini_buffer, image, { quoted: mech, caption: caption })
                                    get_audio = await getBuffer(result.link)
                                    await fdhl.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${result.title}.mp3`, quoted: mech })
                                })
                        })
                    break					
...
//»»»»»»»»»»»»»»»»»»»»»»|| menu + fake reply ||«««««««««««««««««««««««//
                   ...
                   case 'menu':
                   case 'help':
                   inimenu = `Hi  ${pushname} ${ucapanWaktu}
   ≡ *${botname} menu*    
  🎟${prefix}groupmenu
  🎟${prefix}stickermenu
  🎟${prefix}funmenu
  ...
  🎟${prefix}hentaimenu️`	                                  
                   ...
                   { buttonId: '#Darkbot', buttonText: { displayText: 'DARKBOT🏷️' }, type: 1 }
                   ]
                  sendButton(from, inimenu, fakeText, but, {quoted: mech})
                  break
...
//»»»»»»»»»»»»»»»»»»»»»»|| random menu ||«««««««««««««««««««««««//
 ...
 ❑Stalk Menu
  ...
  ❑Muslim Menu	
    ◈ ${prefix}story of the prophet
	◈ ${prefix}prayer intention
	◈ ${prefix}prayer schedule
	◈ ${prefix} 
...
${wmaing}`
	await fdhl.sendMessage(from, amenu, text, {quoted: ftrol})
	               break
                    ...
                    case 'allmenu':
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "𝗕𝗢𝗧 𝗠𝗘𝗡𝗨\n‣ Info darkbot : status online\n‣ Rating : ★★★★☆"
                    var ini_buffer = fs.readFileSync ('./Arin/commands.jpg')
                    const ini_csreply = {
                        contextInfo: {
...
case 'hentaimenu':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Nsfw feature is not active in this group\nTap: ${prefix}nsfw 1 \nTo be active`)
...
OWN SIN!!
  ...
  ❑Hentai Menu
	...
	◈ ${prefix}solo
	◈ ${prefix}erochan`
     fdhl.sendMessage(from, inibuf, image, { caption: text1, quoted: mech })
     break
case 'Darkbot':
...
case 'menu2':
 ...
 footerText: 'Yuri',
 description: `Hai @${sender.split('@')[0]}, Please select the menu here`,
 sections: [
                     {
...
case 'story of the prophet':
      if (args.length < 1) return reply(`What are you looking for bro ${pushname}\nExample ${prefix}${command} muhammad`)
      Text = q
      anu = await fetchJson(`https://hardianto-chan.herokuapp.com/api/muslim/story of the prophet?nabi=${Text}&apikey=hardianto`, {method: 'get'})
 	  reply(`*The story of the Prophet ${Text}*\n\n`+anu.result.kisah)
				break
case ' ':
...
case 'prayer schedule':
      if (args.length < 1) return reply('Region/City?')
      anu = await fetchJson(`https://api.lolhuman.xyz/api/sholat/${body.slice(14)}?apikey=${cikoapi}`, {method: 'get'})
      Text = q
      peb = `*[ PRAYER SCHEDULE ]*
      
*Region : ${anu.result.wilayah}*
*RegionDate : ${anu.result.tanggal}*
*Sohur : ${anu.result.sahur}*
*Imsak : ${anu.result.imsak}*
*Subuh : ${anu.result.subuh}*
*Duhur : ${anu.result.dzuhur}*
*Asr : ${anu.result.ashar}*
*Maghrib : ${anu.result.maghrib}*
*ishaa : ${anu.result.isya}*
`
reply(`Regional Prayer Schedule *${Text}*\n\n`+peb)
				break
case 'prayer intention':
       if (args.length < 1) return reply('Example #prayer intention subuh')
...
//┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅〔 FUN MENU〕┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅//		
            ...
            case 'trut':
                  if (!isGroup) return reply(mess.only.group)
                  const trut =['هل احببت احد من قبل؟ ما المدة؟','إذا استطعت أو إذا أردت ، فمن ستكون صديقًا له علي الانترنت او في الحياة الواقعية؟','ما هي أكبر مخاوفك؟','هل سبق لك أن أحببت شخصًا ما وشعرت أن هذا الشخص معجب بك أيضًا؟ "،" ما هو اسم صديقك السابق الذي أحببته سراً مرة واحدة؟ "،" هل سبق لك أن حاولت رشوة المال؟ والسبب؟ "ما الذي يجعلك سعيدا عندما تكون حزينا" ، "هل سبق لك أن وقعت في الحب من جهة واحدة؟ إذا كان حدث بالفعل اذا مع من؟ ما هو شعورك يا أخي؟ "من هو الشخص الذي يمكن أن يجعلك عظيماً" ، "من هو الشخص الذي جعلك رائعة" ، "من هو الأقرب إلى نوع شريكك المثالي هنا؟']
		...
		case 'dare':
		if (!isGroup) return reply(mess.only.group)
		const dare =['ارسل رسالة الي حبيبك السابق اخبره انك مازلت معجبا به','اتصل باخر شخص في سجل مكالمات الواتس اب','اخبر احد اعضاء المجموعة من الجنس  الاخر انه اجمل ما في الوجود وانت صادق في كل ما تقول','اتصل باحد اعضاء الجروب هاتفيا','أخبر حبيبك بكلمات قاسية في الخاص  ثم أرسلها هنا','أصرخ "أنا غبي" أمام منزلك او نغير اسمك إلى"شقروولي"لمدة 24 ساعة','ارسل صورتك مكتوب فيهاأنا طفل بالتبني','اربط وجهك ثم أرسله إلى أحد أصدقائك','اعترف إلى أحد أعضاء المجموعة','اتصل هاتفيًا بحبيبك الآن','امزح مع حبيبك القديم واخبره انا احبك ارجوك لنعود','سجل صوت صياحك','قل أنا أعشقك هل تريد أن تكون حبيبي الي اخر شخص من الجنس الآخر تحدثت معه ثم انتظر حتى يرد','غير اسمك إلى الطفل لمدة 5 ساعات','أذكر نوع شخصية صديقك','اكتب في اللغة المحلية 24 ساعة','اتصل باخر شخص في سجل مكالمات الواتس اب']
		const der = dare[Math.floor(Math.random() * dare.length)]
		tod = await getBuffer(`https://www.linkpicture.com/q/images_72.png`)
		fdhl.sendMessage(from, tod, image, { quoted: mech, caption: '*DARE*\n\n'+ der })
        		break
            case 'memes':
                    if (!isGroup) return reply(mess.only.group)
					let data = fs.readFileSync('./lib/memes.js');
					jsonData = JSON.parse(data);
					randIndex = Math.floor(Math.random() * jsonData.length);
					randKey = jsonData[randIndex];
					hasil = await getBuffer(randKey.result)
					fdhl.sendMessage(from, hasil, image, {thumbnail: Buffer.alloc(0), quoted: mech})
					break
					//menu gabut gc
//-- ganteng cek
case 'handsome':
  if (args.length < 1) return reply(`Example : ${prefix}handsome`)
if (!isGroup) return reply(mess.only.group)
  random = `${Math.floor(Math.random() * 100)}`
  gan = q
  cek = `Target : *${gan}*
Persentase : ${random}%`
fdhl.sendMessage(from, cek, text, {quoted: mech})
break

//--- cantik cek
case 'beautiful check':
  if (args.length < 1) return reply(`Example : ${prefix}beautiful check pebkun`)
if (!isGroup) return reply(mess.only.group)
  random = `${Math.floor(Math.random() * 100)}`
  can = q
  cek = `Target : *${can}*
...
case 'apakah':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply(`Example : ${prefix}apakah pebkun jelek`)
apa = q
...
case 'rate':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply(`Example : ${prefix}rate pebkun signs`)
rate = q
random = `${Math.floor(Math.random() * 100)}`
rating = `Rate ${rate}
Persentase : ${random}%`
...
case 'bisakah':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply(`Example : ${prefix}can pebkun fly`)
bisa = q
naon = ["can","not","maybe"]
random = naon[Math.floor(Math.random() * (naon.length))]
...
case 'kapankah':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply(`Example : ${prefix}when do pebkun get married`)
kapan = q
no = `${Math.floor(Math.random() * 100)}`
naon = ["More hours","Another day","Another week","Another month","Another year"]
random = naon[Math.floor(Math.random() * (naon.length))]
kapan = `When ${kapan}
Answer : ${no} ${random}`
reply(kapan)
...
case 'tagall':
		  if (!isGroup) return reply(mess.only.group)
		    if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					members_id = []
			Text = `▢ Group : *${groupName}*\n▢ Amount member : *${groupMetadata.participants.length}*\n\n┌───⊷ *MENTIONS* ⊶\n`
			for (let mem of groupMembers) {
						Text += `├╼ @${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
			Text += `└──────────────`
			mentions(Text, members_id, true)
			break
...
fdhl.sendMessage(from, buffer, image, { quoted: mech, thumbnail: fs.readFileSync('./pebrikun.jpg')})
break
case  'solo': 
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Nsfw feature is not active in this group\nTap: ${prefix}nsfw 1 \nTo be active`)
...
case  'waifusearch': 
       if (args.length < 1) return reply(`Example ${prefix}${command} sagiri`)
       reply(mess.wait)
...
case 'ppcp':
			...
			fdhl.sendMessage(from, cowo1, MessageType.image, { thumbnail: fs.readFileSync('./pebrikun.jpg'), caption: 'ini cowo nya..', quoted: mech})
		    cewe1 = await getBuffer(anu1.result.cewe_image)
		    fdhl.sendMessage(from, cewe1, MessageType.image, { thumbnail: fs.readFileSync('./pebrikun.jpg'), caption: 'ini cewe nya...', quoted: mech})
		    break
//»»»»»»»»»»»»»»»»»»»»»»|| PRIMBON MENU ||«««««««««««««««««««««««//

case 'approaching':
      if (args.length < 1) return reply(`Example ${prefix}${command} Pebri`)
      Text = q
      anu = await fetchJson(`https://mnazria.herokuapp.com/api/arti?nama=${Text}`, {method: 'get'})
 	  reply(`*Arti Nama ${Text}*\n\*`+anu.result)
				break

//┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅〔 Downloader Menu 〕┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅//	

				case 'tiktokdl': 
				if (args.length < 1) return reply('Where is the url dear?')
...
case 'ytlink':
if (args.length < 1) return reply('Where is the link dear?')
if(!ini_url(args[0]) && !args[0].includes('youtu')) return reply('error mass')
Text = args.join(' ')
reply(mess.wait)
res = await y2mateV(Text).catch(e => {
reply('_[ ! ] Error Failed to Enter Y2mate_ Web')
})
result = `❒「  *${botname}*  」
├ *Title :* ${res[0].judul}
├ *size :* ${res[0].size}
├ *Quality :* ${res[0].quality}p
├ *Nama File :* ${res[0].output}
└ *Output :* ${res[0].tipe}

_*Wait for the Uploading Process.....*_
`
sendFileFromUrl(res[0].thumb, image, {caption: result, quoted: mech}).then((lalu) => {
sendFileFromUrl(res[0].link, video, {quoted: mech, mimetype: 'video/mp4', filename: res[0].output})
})
break
//┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅〔 StalkingMenu 〕┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅//	

case 'github':
      if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} febzofc\nMake sure Github Name is Correct`)
      ini_txt = args.join(" ")
      reply(mess.wait)
...
*Bio :* ${peb.result.bio}
`
fdhl.sendMessage(from, buffer, image, {quoted: mech, caption: Text})
break
case 'instagram':
      if (args.length == 0) return reply(`Where's the text bro? ${pushname}\nExample: ${prefix + command} febzofc\nMake sure instgram Name is Correct`)
      ini_txt = args.join(" ")
      reply(mess.wait)
...
*title : ${tod.result.title}*
*Episode : ${tod.result.total_episode}*
*released : ${tod.result.released_on}*
*Genre : ${tod.result.genre}*
*Duration : ${tod.result.duration}*
...
`
fdhl.sendMessage(from, buffer, image, {quoted: mech, caption: peb})
//reply(`ANIME BATCH *${Text}*\n\n`+peb)
break
              ...
              case 'otaku':
                    ...
                    break	
                case 'wait':
                    if ((isMedia && !mech.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mech).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mech
                        ...
                        await fdhl.sendMessage(from, ini_video, video, { quoted: mech, caption: ini_txt })
                    } else {
                        reply(`Send a picture with a caption ${prefix + command} or tag images that have already been sent`)
                    }
                    break                    																			
//┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅〔 FEATURE GROUP 〕┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅//
     ...
     case 'welcome':   
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
         if (args.length < 1) return reply(`Tap :\n${prefix}welcome on to activate\n${prefix}welcome off to disable`)
         if ((args[0]) === 'on') {
         if (isWelkom) return reply('welcome is active')
         _welkom.push(from)
         fs.writeFileSync('./database/welcome.json', JSON.stringify(_welkom))
         reply(`\`\`\`✓Successfully activate the welcome feature in the group\`\`\` *${groupMetadata.subject}*`)
         } else if ((args[0]) === 'off') {
     ...
     case 'antilink' :
         ...
         sendButton(from, "Please select for antilink group", fakeText, but, mech)
         break
     case 'antilinkon' :
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
     ...
     case 'antivirtex' :
         ...
         sendButton(from, "Please select for antivirtex group", fakeText, but, mech)
         break
     case 'antivirtexon' :
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply("Bot not Admin :)")
         if (isAntiVirtex) return reply('anti virtex group was active before')
         _antivirtex.push(from)
         fs.writeFileSync('./database/antivirtex.json', JSON.stringify(_antivirtex))
         reply(`\`\`\`Successfully activated the anti virtex mode in the group\`\`\` *${groupMetadata.subject}*`)
         break
     case 'antivirtexoff' :
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply("Bot not Admin :)")
         if (!isAntiVirtex) return reply('Anti virtex mode was inactive before')
         _antivirtex.splice(from, 1)
         fs.writeFileSync('./database/antivirtex.json', JSON.stringify(_antivirtex))
         reply(`\`\`\`✓Successfully disable anti-virtex mode in the group\`\`\` *${groupMetadata.subject}*`)
         break
     case 'group' :
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isGroup) return reply(mess.only.group)
         ...
         sendButton(from, "Please choose to open/close the group", fakeText, but, mech)
         break
     case 'groupbuka' :
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
     ...
     case 'promote' :
         ...
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply("Bot not Admin :)")
         if (mech.message.extendedTextMessage === undefined || mech.message.extendedTextMessage === null) return reply('Tag the member you want to be an admin!')
         mentioned = mech.message.extendedTextMessage.contextInfo.mentionedJid
         if (mentioned.length > 1) {
         Text = 'Order received, you become admin :\n'
         for (let _ of mentioned) {
         Text += `@${_.split('@')[0]}\n`
         }
         mentions(Text, mentioned, true)
     ...
     case 'demote' :
         ...
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply("Bot not Admin :)")
         if (mech.message.extendedTextMessage === undefined || mech.message.extendedTextMessage === null) return reply('Tag the member that you want to not be an admin!')
         mentioned = mech.message.extendedTextMessage.contextInfo.mentionedJid
         if (mentioned.length > 1) {
         Text = 'Order received, you are not an admin :\n'
         for (let _ of mentioned) {
         Text += `@${_.split('@')[0]}\n`
         }
         mentions(Text, mentioned, true)
         ...
         mentions(`Order received, Lowering : @${mentioned[0].split('@')[0]} Become a Member`, mentioned, true)
         fdhl.groupDemoteAdmin(from, mentioned)
         }
         break
     case 'add' :
         ...
         if (args[0].startsWith('08')) return reply('Use the country code Gan')
         try {
     ...
     case 'kick' :
         ...
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply("Bot not Admin :)")
         if (mech.message.extendedTextMessage === undefined || mech.message.extendedTextMessage === null) return reply('Tag the target you want to kick!')
         mentioned = mech.message.extendedTextMessage.contextInfo.mentionedJid
         if (mentioned.length > 1) {
         Text = 'Order received, remove :\n'
         for (let _ of mentioned) {
         Text += `@${_.split('@')[0]}\n`
         }
         mentions(Text, mentioned, true)
         ...
         mentions(`Order received, remove : @${mentioned[0].split('@')[0]}`, mentioned, true)
         fdhl.groupRemove(from, mentioned)
         }
         break
     case 'setname':
         ...
         fdhl.sendMessage(from, `\`\`\`✓Successfully changed the name of the group to be\`\`\` *${body.slice(9)}*`, text, { quoted: ftrol })
         break
     case 'setdesc':
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
         ...
         fdhl.sendMessage(from, `\`\`\`✓Successfully replacing the Group Description\`\`\` *${groupMetadata.subject}* Being: *${body.slice(9)}*`, text, { quoted: ftrol })
         break
     case 'setpp':
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
         if (!isBotGroupAdmins) return reply(mess.only.Badmin)
         media = await fdhl.downloadAndSaveMediaMessage(mech, './database/media_user')
         await fdhl.updateProfilePicture(from, media)
         reply(mess.wait)
         reply(`\`\`\`✓Successfully replace the Group Profile\`\`\` *${groupMetadata.subject}*`)
         break
     case 'hidetag':
         if (!isGroup) return reply(mess.only.group)
         if (!isGroupAdmins) return reply(mess.only.admin)
...
//━━━━━━━━━━━━━━━[ FITUR STICKER ]━━━━━━━━━━━━━━━━━//
     ...
     case 'attp':
         if (args.length == 0) return reply(`Example: ${prefix + command} Hi`)
     ...
     case 'tovideo':
         ...
         reply(mess.wait)
         anumedia = JSON.parse(JSON.stringify(mech).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
         anum = await fdhl.downloadAndSaveMediaMessage(anumedia, './database/media_user')
         ran = getRandom('.webp')
         exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
         fs.unlinkSync(anum)
         buffer = fs.readFileSync(ran)
         fdhl.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done...' })
         fs.unlinkSync(ran)
         })
         break
     case 'donasi':
...
//━━━━━━━━━━━━━━━[ FITUR OWNER ]━━━━━━━━━━━━━━━━━//
     ...
     case 'owner':
         ...
         reply('That is the owner number')
         break
     case 'bc':
         if (!isOwner) return reply('LU not OWNER GBLOK')
         if (args.length < 1) return reply('.......')
         anu = await fdhl.chats.all()
         if (isMedia && !mech.message.videoMessage || isQuotedImage) {
         const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mech).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mech
         bc = await fdhl.downloadMediaMessage(encmedia)
         for (let _ of anu) {
         fdhl.sendMessage(_.jid, bc, image, { caption: `[ Dark Bot\nIzin Broadcast ]\n\n${body.slice(4)}` })
         }
     ...
     case 'report':
         ...
         if (pesan.length > 300) return pras.sendMessage(from, 'Sorry Too Long Text, Maximum 300 Texts', text, { quoted: ftrol })
         var nomor = mech.participant
     ...
     default:
                    /* if (isCmd) {
                        reply(`Sorry bro, command *${prefix}${command}* not on the list *${prefix}menu*`)
                    }
     ...
     if (isOwner) {
			     if (budy.startsWith('>')) {
				     console.log(color('[EVAL1]'), color(moment(mech.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`eval return`))
				     try {
				     ...
				     console.log(color('[EVAL2]'), color(moment(mech.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`eval identy`))
				     try {
